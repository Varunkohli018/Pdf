...
function shareFile(index) { const files = JSON.parse(localStorage.getItem("pdf_files") || "[]"); const file = files[index]; fetch(file.data) .then(res => res.blob()) .then(blob => { const fileToShare = new File([blob], file.name, { type: "application/pdf" }); if (navigator.share) { navigator.share({ title: file.name, files: [fileToShare] }); } else { alert("Sharing not supported on this device."); } }); }
